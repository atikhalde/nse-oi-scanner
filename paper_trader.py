...
✅ Entry logic: Exact same as full_telegram_alerts.py (22 conditions + exceptions)
...
✅ Target 1:2 (primary)
✅ Trail SL to breakeven after hitting 1:2
✅ After hitting 1:3 → trail SL to Cost-To-Cost (entry price)
...
PAPER_QTY = 1               # paper quantity (you can change)
...
def get_full_signal(symbol):
    ...
    df['retest_ok'] = ((df['low'] >= df['breakout_level'] * 0.997) & 
                       (df['low'] <= df['breakout_level'] * 1.003) & 
                       (df['close'] > df['breakout_level'])).astype(int)
    ...
    trend_base = np.where((df['close'] > df['ema20']) & (df['ema20'] > df['ema50']), 20,
    ...
    vol_part = np.where((df['obv_slope'] > 0) & (df['rel_vol'] > 1.3), 15,
    ...
    exception_buy = (high_base or only_cond22_failed or strong_exception) and score >= 48 and close_above_vwap and (last['rel_vol'] or 0) > 0.85 and vwap_not_far
    normal_buy = close_above_ema and close_above_vwap and (base >= 9 or (base >= 6 and rsi > 60) or (score >= 45 and rsi > 57))

    if exception_buy or normal_buy:
        return "BUY", f"base={base}/22 score={score}", entry_time_str, last['close'], last['low']   # return entry_price + entry_candle_low as SL

    # SELL side (simplified for paper)
    normal_sell = (not close_above_ema) and (base <= 7 or rsi < 47 or (last['close'] < last['ema20'] and rsi < 50))
    if normal_sell:
        return "SELL", f"base={base}/22 score={score}", entry_time_str, last['close'], last['high']

    return None, f"base={base}/22 score={score}", None, None, None

# ==================== PAPER TRADING ENGINE ====================
def calculate_paper_pnl(side, entry_price, exit_price, qty=PAPER_QTY):
    if side == "BUY":
...
def manage_paper_positions():
    ...
    for symbol in list(positions.keys()):
        ...
        target1 = pos['target1']
        target2 = pos['target2']
        entry_candle_low = pos.get('entry_candle_low')
        entry_candle_high = pos.get('entry_candle_high')
        ...
        if side == "BUY":
            ...
            if current_low <= sl:
                exit_price = sl
                reason = "SL HIT"
            # Check if 1:3 reached → trail to CTC
            elif current_high >= target2 and not pos.get('tp3_hit'):
                pos['tp3_hit'] = True
                pos['sl'] = entry_price   # trail to cost-to-cost
                print(f"   🎯 {symbol} BUY hit 1:3 → SL trailed to CTC (entry) @ {entry_price:.2f}")
                continue
            # Check 1:2 reached → trail to breakeven
            elif current_high >= target1 and not pos.get('tp1_hit'):
                pos['tp1_hit'] = True
                pos['sl'] = entry_price
                print(f"   🎯 {symbol} BUY hit 1:2 → SL moved to breakeven (entry)")
                continue
            # Normal target hit after trailing
            elif current_high >= target1 and pos.get('tp3_hit'):
                exit_price = target2   # or current_close if you prefer
                reason = "TP 1:3 (after trailing)"
            elif current_high >= target1:
                exit_price = target1
                reason = "TP 1:2"

            # Update trailing SL if price is making new highs after breakeven
            if pos.get('tp1_hit') and current_close > entry_price:
                # Optional simple trail: move SL up with price (here we keep at entry for CTC after 1:3)
                pass
            ...
            if current_high >= sl:
                exit_price = sl
                reason = "SL HIT"
            elif current_low <= target2 and not pos.get('tp3_hit'):
                pos['tp3_hit'] = True
                pos['sl'] = entry_price
                print(f"   🎯 {symbol} SELL hit 1:3 → SL trailed to CTC")
                continue
            elif current_low <= target1 and not pos.get('tp1_hit'):
                pos['tp1_hit'] = True
                pos['sl'] = entry_price
                print(f"   🎯 {symbol} SELL hit 1:2 → SL to breakeven")
                continue
            elif current_low <= target1:
                exit_price = target1
                reason = "TP 1:2"
        ...
        if exit_price is not None and reason is not None:
            ...
            pnl = calculate_paper_pnl(side, entry_price, exit_price)
...
if __name__ == "__main__":
    ...
    if run_once:
        ...
        # Exit only if completely outside full market hours
        if now.weekday() >= 5 or not ("09:15" <= current <= "15:30"):
            print(f"[{current} IST] Outside market hours (09:15-15:30). Exiting.")
            sys.exit(0)
        ...
        # === IMMEDIATE DIAGNOSTIC (even outside window) ===
        print("   → Sending immediate diagnostic message to Telegram...")
        diag = (
            "🧪 *PAPER TRADER TEST* (RUN_ONCE)\n\n"
            f"Time: {now.strftime('%H:%M IST')}\n"
            f"Inside market hours: {'09:15' <= current <= '15:30'}\n"
            f"Inside entry window: False (early morning test)\n\n"
            "If you see this → Bot + Secrets are WORKING from Combined job!"
        )
        send_telegram(diag)
