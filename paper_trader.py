...
✅ Max **5 entries per stock per day** (multiple signals allowed)
...
MAX_ENTRIES_PER_STOCK = 5   # Max 5 paper entries per stock **per day**
...
# Daily paper entry tracking (Max 5 per stock per calendar day)
_daily_entry_date = None
_daily_entry_counts = {}    # symbol -> count for current IST date

def _get_today_ist():
    return datetime.now(IST).strftime("%Y-%m-%d")

def get_daily_paper_entry_count(symbol):
    """Returns how many paper entries have been taken for this symbol today."""
...
def increment_daily_paper_entry_count(symbol):
    """Increments the daily paper entry counter for the symbol."""
    global _daily_entry_date, _daily_entry_counts
    today = _get_today_ist()
    if _daily_entry_date != today:
        _daily_entry_date = today
        _daily_entry_counts.clear()
    _daily_entry_counts[symbol] = _daily_entry_counts.get(symbol, 0) + 1
...
def open_paper_position(symbol, side, entry_time, entry_price, entry_candle_extreme):
    ...
    # === Daily limit: Max 5 paper entries per stock per IST calendar day ===
    daily_count = get_daily_paper_entry_count(symbol)
    if daily_count >= MAX_ENTRIES_PER_STOCK:
        print(f"   🚫 {symbol} reached daily max {MAX_ENTRIES_PER_STOCK} paper entries")
        return False
...
def run_paper_trader():
    ...
    print("   Max 5 entries per stock per day (daily reset)")
    print("=" * 70)
