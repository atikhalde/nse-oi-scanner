...
✅ Max **5 entries per day** on **DIFFERENT stocks only** (1 entry per stock max)
...
MAX_DAILY_ENTRIES = 5          # Max total paper entries per day (across different stocks)
MAX_ENTRIES_PER_STOCK = 1      # 1 entry per stock per day (must be different stocks)
...
# Daily paper entry tracking (Max 5 DIFFERENT stocks per calendar day; 1 entry per stock)
_daily_entry_date = None
_daily_entry_counts = {}    # symbol -> count for current IST date (now always 0 or 1)
_daily_total_entries = 0    # total unique stocks entered today (max 5)
...
def get_daily_paper_entry_count(symbol):
    ...
    global _daily_entry_date, _daily_entry_counts, _daily_total_entries
    today = _get_today_ist()
    if _daily_entry_date != today:
        _daily_entry_date = today
        _daily_entry_counts.clear()
        _daily_total_entries = 0
    return _daily_entry_counts.get(symbol, 0)

def get_daily_total_paper_entries():
    """Returns total number of different stocks entered today (max 5)."""
    global _daily_entry_date, _daily_entry_counts, _daily_total_entries
    today = _get_today_ist()
    if _daily_entry_date != today:
        _daily_entry_date = today
        _daily_entry_counts.clear()
        _daily_total_entries = 0
    return len(_daily_entry_counts)

def increment_daily_paper_entry_count(symbol):
    """Increments the daily paper entry counter for the symbol (caps at 1 per stock; total counts unique)."""
    global _daily_entry_date, _daily_entry_counts, _daily_total_entries
    today = _get_today_ist()
    if _daily_entry_date != today:
        _daily_entry_date = today
        _daily_entry_counts.clear()
        _daily_total_entries = 0
    prev_count = _daily_entry_counts.get(symbol, 0)
    if prev_count == 0:
        _daily_entry_counts[symbol] = 1
        _daily_total_entries += 1
    # if already 1, do nothing (idempotent)
...
def open_paper_position(symbol, side, entry_time, entry_price, entry_candle_extreme):
    ...
    # === Daily limit: Max 5 DIFFERENT stocks per day (1 entry per stock) ===
    ...
    total_today = get_daily_total_paper_entries()
    if total_today >= MAX_DAILY_ENTRIES:
        print(f"   🚫 Reached daily max of {MAX_DAILY_ENTRIES} different stocks today. No more entries allowed.")
        return False

    print(f"   📊 Daily counters: {symbol} has {daily_per_stock} today | Total different stocks today: {total_today}/{MAX_DAILY_ENTRIES}")
...
def run_paper_trader():
    ...
    print("   Max 5 entries per day on DIFFERENT stocks only (1 per stock)")
    print("=" * 70)
