...
# ---------------------------------------------------------------- M9 gainers gate
# M9 cash data-farm entry gate (user pick A, 29-Jul-2026): a BUY master signal
# may enter ONLY while the stock sits on NSE's live "top gainers — ALL
# SECURITIES" list (nseindia.com/market-data/top-gainers-losers), top-20 by
# % change vs previous close. Direction-aligned movers gate, like M2's BUY side.
# Strict mode: an unreachable feed passes NOTHING that cycle (logged, retried).
GAINERS_URLS = [
    "https://www.nseindia.com/api/live-analysis-variations?index=gainers",
    "https://www.nseindia.com/api/live-analysis-gainers",
    "https://www.nseindia.com/api/live-analysis-gainers?type=equity",
]
GAINERS_PRIME = "https://www.nseindia.com/market-data/top-gainers-losers"

_GSYM = ("symbol", "Symbol", "SYMBOL", "tradingSymbol", "trading_symbol")
_GPCT = ("per_change", "perChange", "pChange", "pchange", "pctChange", "pct_change",
         "changePct", "change_pct")


def _sym_pct_lists(node, out):
    """Recursively collect lists-of-dicts that look like symbol/pct-change rows."""
    if isinstance(node, dict):
        for v in node.values():
            _sym_pct_lists(v, out)
    elif isinstance(node, list):
        if node and all(isinstance(x, dict) for x in node):
            if any(any(k in x for k in _GSYM) for x in node) and \
               any(any(k in x for k in _GPCT) for x in node):
                out.append(node)
        for v in node:
            _sym_pct_lists(v, out)


def _gainers_from_json(j, top_n):
    """Any NSE gainers payload -> (top-N symbol set, total rows parsed). The
    all-securities category is always the LARGEST symbol/pct list in the payload."""
    lists = []
    _sym_pct_lists(j, lists)
    if not lists:
        return None, 0
    ...
    if not rows:
        return None, 0
    rows.sort(key=lambda x: -x[1])
    return {s for s, _ in rows[:top_n]}, len(rows)


def nse_movers_gainers(top_n=20):
    """Live NSE top-gainers (all-securities) snapshot -> (top-N set, meta).
    STRICT: an empty set means the feed is offline — nothing passes."""
    # 1) curl_cffi — real browser TLS fingerprint (same ladder as the OI gate)
    try:
        s = _curlcffi_session()
        try:
            s.get(GAINERS_PRIME, timeout=20)      # cookie prime for the page
        except Exception:
            pass
        for u in GAINERS_URLS:
            for _ in range(3):
                try:
                    r = s.get(u, timeout=15)
                    if r.status_code != 200:
                        time.sleep(1.5); continue
                    syms, n_all = _gainers_from_json(r.json(), top_n)
                    if syms:
                        return syms, {"status": "OK", "source": u + " (curl-cffi)",
                                      "count": n_all, "top_n": top_n}
                except Exception:
                    time.sleep(1.5)
    except Exception:
        pass
    ...
    for u in GAINERS_URLS:
        for _ in range(2):
            try:
                r = s.get(u, headers={"Referer": GAINERS_PRIME}, timeout=20)
                if not r.ok:
                    continue
                syms, n_all = _gainers_from_json(r.json(), top_n)
                if syms:
                    return syms, {"status": "OK", "source": u,
                                  "count": n_all, "top_n": top_n}
            except Exception:
                time.sleep(2)
    return set(), {"status": "OFFLINE", "source": "nse top-gainers (all-sec)",
                   "count": 0, "top_n": top_n}
