...
def mode_live():
    ...
    for tkey in list(st["trades"].keys()):
        ...
        tr = st["trades"][tkey]
        try:
            new_tr = trader.evaluate(sym, tr["side"], tr["time"], float(tr["entry"]), tr["signal"], tbars, warmup=trader.load_warmup(L.HIST / f"{sym}.csv", today))
    ...
    if hhmm >= "15:25":
        try:
            ...
            out = report.build(done, dlbl, st["gate"], str(L.ROOT / f"paper_test_M2_{today}.xlsx"), skipped=sk or None)
