...
Rules / engine / sizing: identical to Model 1 (intact master engine, structure SL
∓0.02%, NO fixed targets — +1R arms the setup-aware trail, 15:20 square-off,
₹50,000 per stock). Blocked weak-EX signals are GHOST-shadowed (dual-SL, EOD only)
exactly like Model 1 — see live_runner.py.
            ...
            try:
                ...
                if not pd.isna(code) and int(code) in L.MASTER_CODES:
                    ...
                    if int(code) in (90, 290):
                        print(f"  M2 {sym} {side} @ {etime} — SKIPPED: scanner-table preview 90/290 (no chart label)")
                        ...
                        print(f"  M2 {sym} {side} @ {etime} — SKIPPED: weak EX variant (EX9+) quality filter")
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                        ...
                        if etime >= L.CHART_MIN_TIME:    # ghost: shadow the release candidate
                        ...
                        print(f"  M2 {sym} {side} @ {etime} — SKIPPED: EX1/EX2 at open (razor class, allowed from {L.EX_OPEN_FROM})")
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": f"EX1/EX2 before {L.EX_OPEN_FROM} blocked — razor-edge class (B2)"})
                        skipped_quality += 1
                        time.sleep(0.5)
                    elif etime < L.CHART_MIN_TIME:
                        print(f"  M2 {sym} {side} @ {etime} — SKIPPED: chart window (<{L.CHART_MIN_TIME})")
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": f"signal before {L.CHART_MIN_TIME} chart window (not on TradingView)"})
                        skipped_early += 1
                        time.sleep(0.5)
                    elif not gate_ok:
                        ...
                        print(f"  M2 {sym} {side} @ {etime} — SKIPPED: {sym} trade already open (1-open-trade rule)")
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                        ...
                        tr = trader.evaluate(sym, side, etime, entry, str(row.get("scan_name", code)), tbars, warmup=trader.load_warmup(L.HIST / f"{sym}.csv", today))
                        if "error" in tr:
                            print(f"  M2 {sym} {side} @ {etime} — trader rejected: {tr.get('error')}")
                        else:
