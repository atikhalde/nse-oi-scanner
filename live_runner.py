...
import costs, feeds, gate, trader, report, telegram_bot as tg
import learn_log
...
EX_OPEN_FROM = "09:45"
# GHOST SHADOW (user, 24-Jul: "Choose A"): the weak family STAYS BLOCKED live —
# releasing it with prev-candle SL lost Rs1,592 in the 3-day A/B — but every
# blocked weak chart signal is also paper-evaluated after the close in BOTH stop
# styles (structure SL and prev-candle low/high ∓0.02%, the user's release spec)
# with full costs+slippage. Zero exposure, no intraday alerts; the EOD report
# carries a GHOST section and the release decision is revisited on ~10 sessions
# of side-by-side evidence instead of one disaster day.
...
def run_ghosts(st, today, bars_map):
    """GHOST shadow eval (EOD only, user 24-Jul): every weak-EX signal that was
    BLOCKED live is paper-traded after the close in BOTH stop styles —
    structure SL and prev-candle low/high ∓0.02% — with full costs+slippage.
    Pure shadow: no orders, no intraday alerts, EOD-report evidence only."""
    out, seen = [], set()
    for g in st.get("ghosts", []):
        key = (g["symbol"], g["side"], g["time"], g["signal"])
        if key in seen:
            continue
        seen.add(key)
        row = dict(g)
        tbars = bars_map.get(g["symbol"])
        for mode in ("structure", "prevbar"):
            row[f"net_{mode}"] = row[f"exit_{mode}"] = row[f"r_{mode}"] = None
            if tbars is None:
                continue
            try:
                tr = trader.evaluate(g["symbol"], g["side"], g["time"], float(g["entry"]), g["signal"],
                                     tbars, warmup=trader.load_warmup(HIST / f"{g['symbol']}.csv", today),
                                     sl_mode=mode)
                if "error" in tr:
                    row[f"exit_{mode}"] = f"n/a ({tr['error']})"
                    continue
                row[f"net_{mode}"] = costs.trade_costs(tr)["net"]
                row[f"exit_{mode}"] = tr["exit_text"]
                row[f"r_{mode}"] = tr["r_total"]
            except Exception as e:
                print(f"  ghost {g['symbol']} {mode}: {type(e).__name__}: {e}")
        out.append(row)
    return out
            ...
            try:
                ...
                if not pd.isna(code) and int(code) in MASTER_CODES:
                    ...
                    if int(code) in (90, 290):
                        ...
                        # (telegram skip-alert removed 25-Jul: only paper-trade alerts stay loud;
                        #  full skip detail remains in Actions logs + EOD xlsx + state/learn logs)
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": "scanner-table preview signal (90/290) — no TradingView chart label, blocked"})
                        skipped_table += 1
                        time.sleep(0.5)
                        ...
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": "weak EX variant (EX9+) blocked — B2 quality filter"})
                        if etime >= CHART_MIN_TIME:      # ghost: shadow the release candidate
                        ...
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": f"EX1/EX2 before {EX_OPEN_FROM} blocked — razor-edge class (B2)"})
                        skipped_quality += 1
                        time.sleep(0.5)
                        ...
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": f"signal before {CHART_MIN_TIME} chart window (not on TradingView)"})
                        skipped_early += 1
                        time.sleep(0.5)
                    elif not gate_ok:
                        ...
                        st.setdefault("skipped", []).append(
                            {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                             "time": etime, "entry": round(entry, 2), "rank": (int(rank) if rank is not None else None), "why": "open position already on stock (1-open-trade rule)"})
                        skipped_open += 1
                        time.sleep(0.5)
                    else:
...
def mode_replay(date, trade_csv):
    ...
    for _, r in pd.read_csv(trade_csv).iterrows():
        ...
        tr = trader.evaluate(sym, r["side"], r["time"], float(r["entry"]), r.get("signal", ""), b,
                         ...
                         sl_mode=(str(r["sl_mode"]) if "sl_mode" in r and pd.notna(r["sl_mode"]) else "structure"))
