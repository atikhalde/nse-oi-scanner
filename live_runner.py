...
def fmt_skipped(sym, side, name, etime, entry, tag=""):
    """Alert for a gated master signal that is NOT traded because the stock
    already has an open paper trade. Sent once per signal; info-only."""
    return (f"{tag}⏭️ <b>NO NEW TRADE — {sym} {side}</b>\n"
            f"{name} @ {etime} · ₹{entry:,.2f}\n"
            f"🚦 {sym} trade already open — 1-open-trade-per-stock rule")
...
def mode_live():
    ...
    print(f"cycle done: {len(st['trades'])} trades · {len(bars_map)} symbols fed · "
          ...
          f"skipped-open +{skipped_open}) · gate {'OK' if gate_ok else 'OFFLINE'}")
    return True
