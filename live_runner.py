...
def mode_live():
    ...
    for sym, tbars in bars_map.items():
        ...
        try:
            for idx, row in today_res.iloc[known:].iterrows():
                ...
                px = df["close"].loc[idx]          # engine output has no OHLC cols;
                entry = float(px.iloc[-1] if hasattr(px, "iloc") else px)   # price from input frame
                rank = ranks.get(sym)
                if not gate_ok:
    ...
    # --- quiet-cycle heartbeat: silent Telegram line every 30 min of pure silence
    #     (raw + counted alerts both considered), so "no alerts" is verifiably "nothing happened"
    new_tg = (len(st["alerts"]) - alerts_before) + raw_new
    if new_tg == 0 and "09:20" <= hhmm < "15:25":
        last_hb = st.get("last_hb")
        if not last_hb or (dt.datetime.strptime(hhmm, "%H:%M")
                           - dt.datetime.strptime(last_hb, "%H:%M")).seconds >= 1800:
            tg.send_message(f"💓 alive {hhmm} IST · {len(st['trades'])} trades today · "
                            f"no new signals since last alert · gate "
                            f"{'OK' if gate_ok else 'OFFLINE'}", silent=True)
            st["last_hb"] = hhmm

    st["cycles"] += 1
    save_state(st)
    print(f"cycle done: {len(st['trades'])} trades · {len(bars_map)} symbols fed · "
          f"TG alerts today: {len(st['alerts'])} (+{len(st['alerts']) - alerts_before}, raw +{raw_new}) · gate "
