#!/usr/bin/env python3
"""MODEL 3 paper trader — OPTIONS BUYING mirror of live_runner (M1).

Entries: IDENTICAL to M1 — intact master engine, 09:26 chart-window gate,
90/290 table-preview block, B2 quality filter (EX9+ blocked, EX1/EX2 from 09:45),
TOP-30 NSE OI-spurt gate, 1-open-trade-per-stock. Every filter byte-shared with
live_runner.py's decision chain.

Position: nearest-expiry 1st-ITM option BUY (CE for BUY, PE for SELL).
  SL prem = entry_prem − delta×UL-risk (UL signal-bar low/high ∓0.02%, set once)
  skip if 1-lot risk > ₹2,000 (₹1,900 planned) · premium outlay ≤ ₹50,000
  TP1 1:1 → book 50% · rest trails underlying structure swings · sq-off 15:20.

Separate ledger (state3.json) + workflow (5. LIVE M3). Alerts tagged 🅼3.
Usage: python -u m3_runner.py --live --loop 2
"""
import argparse
...
import feeds, gate, report, telegram_bot as tg
import options_common as optx   # options paper engine (M3)

IST = dt.timezone(dt.timedelta(hours=5, minutes=30))
STATE3 = ROOT / "state3.json"
...
def load_state(today):
    if STATE3.exists():
        st = json.loads(STATE3.read_text())
...
def save_state(st):
    STATE3.write_text(json.dumps(st, indent=1))
...
def mode_live():
    ...
    for tkey in list(st["trades"].keys()):
        ...
        try:
            obars = optx.today_opt_bars(tr, now)    # None -> BS synth marks (labeled)
            ...
            for ev in new_tr["events"]:
                ...
                if ev["key"] != "ENTRY" and key not in st["alerts"]:
                    tg.send_message(optx.fmt_opt_alert(new_tr, ev["key"], tag="🅼3"))
            ...
            try:
                ...
                if not pd.isna(code) and int(code) in MASTER_CODES:
                    ...
                    if int(code) in (90, 290):
                        ...
                        tr = optx.enter(sym, side, etime, entry, str(row.get("scan_name", code)), tbars, now, sid=SID.get(sym), hist_csv=HIST / f"{sym}.csv")
                        if "error" in tr:
                            print(f"  {sym} {side} @ {etime} — options rejected: {tr.get('error')}")
                            st.setdefault("skipped", []).append(
                                {"symbol": sym, "side": side, "signal": str(row.get("scan_name", code)),
                                 "time": etime, "entry": round(entry, 2),
                                 "why": f"options rejected: {tr.get('error')}"})
                        else:
                            ...
                            tg.send_message(optx.fmt_opt_alert(tr, "ENTRY", tag="🅼3") +
    ...
    if hhmm >= "15:25":
        try:
            ...
            out = report.build(done, dlbl, st["gate"], str(ROOT / f"paper_test_M3_{today}.xlsx"), skipped=sk or None, rules_note=optx.OPT_RULES_NOTE)
            tg.send_message("🅼3 " + report.summary_text(done, dlbl, st["gate"]))
            tg.send_document(out, caption=f"📄 🅼3 OPTIONS paper report {today}")
            st["eod_done"] = True
        except Exception as e:
    ...
    if "09:20" <= hhmm < "15:26":
        tg.send_message(f"💓🅼3 {hhmm} IST · {len(st['trades'])} trades · alerts {len(st['alerts'])} · "
...
def mode_replay(date, trade_csv):
    print("M3: replay lives in .replay_tmp (options); use --live"); return
    import trader
    trades = []
