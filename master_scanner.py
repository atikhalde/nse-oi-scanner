...
def pine_rma(src: np.ndarray, length: int) -> np.ndarray:
    ...
    Pine-exact seeding: `rma := na(rma[1]) ? ta.sma(src, length) : alpha*src + (1-alpha)*rma[1]`
    -> stays na until `length` non-na values exist, seeded with their SMA
    (NOT with the first value).  Used by ta.rsi / ta.atr / the script's ADX."""
    alpha = 1.0 / float(length)
    sma_seed = pine_sma(src, length)
    out = np.full(len(src), NAN)
    prev = NAN
    for i, x in enumerate(src):
        if math.isnan(prev):
            if not math.isnan(sma_seed[i]):
                prev = sma_seed[i]
        else:
            prev = x if math.isnan(x) else alpha * x + (1.0 - alpha) * prev
        out[i] = prev
...
class _Inputs:
    ...
    def __init__(self, df: pd.DataFrame, p: Params,
        ...
        # Pine ta.tr(handle_na=true): on bar 0 close[1] is na -> tr = high - low
        tr[0] = hl[0]
        ...
        up_move[1:] = h[1:] - h[:-1]
        dn_move[1:] = l[:-1] - l[1:]
        # Pine: (upMove > downMove and upMove > 0) ? upMove : 0  -> na comparisons
        # are false, so bar 0 yields 0.0 (not na).
        ...
        # Pine: DI values are 0.0 while atr is na (rma not seeded yet) -> dx is 0.0,
        # not na; the final rma itself handles its own SMA seeding window.
...
def run_symbol(df: pd.DataFrame, params: Optional[Params] = None,
    ...
    for i in range(n):
        ...
        # Pine: is_first = in_session and not in_session[1]; in_session[1] is na on
        # dataset bar 0 and `not na` is True -> is_first can be true on bar 0.
        is_first = in_session and ((i == 0) or (not I.orb_in_session[i - 1]))
        if i == 0:
