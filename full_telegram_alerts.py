...
✅ (Paper trader side) MAX SL risk capped at 1% for safety
...
# Paper trading settings (for parity with paper_trader.py)
    ...
    for chat_id in chat_ids:
        ...
        try:
            ...
            if r.status_code == 200:
                print(f"   ✅ Telegram message sent successfully (chat_id={chat_id})")
            else:
                print(f"   ❌ Telegram FAILED for chat_id={chat_id} (status={r.status_code})")
                print(f"      Response: {r.text[:200]}")
                all_success = False
        except Exception as e:
            print(f"   ❌ Telegram EXCEPTION for chat_id={chat_id}: {e}")
            all_success = False
...
def get_strong_oi_stocks():
    ...
    try:
        ...
        return pd.DataFrame([
            ...
            {"symbol": "TCS", "avgInOI": 7.5, "changeInOI": 3200},
            {"symbol": "INFY", "avgInOI": 6.9, "changeInOI": 2900},
            {"symbol": "POLYCAB", "avgInOI": 15.4, "changeInOI": 6500},
            {"symbol": "HAVELLS", "avgInOI": 10.1, "changeInOI": 4100},
            {"symbol": "EXIDEIND", "avgInOI": 13.7, "changeInOI": 8900},
            {"symbol": "TECHM", "avgInOI": 8.6, "changeInOI": 3700},
...
def fetch_5m_data(symbol: str):
    try:
        ...
        if len(df) >= 25:
            ...
            # Ensure IST (yfinance .NS returns Asia/Kolkata)
            if df['timestamp'].dt.tz is None:
                df['timestamp'] = df['timestamp'].dt.tz_localize('Asia/Kolkata')
            else:
                df['timestamp'] = df['timestamp'].dt.tz_convert('Asia/Kolkata')

            # CRITICAL: Drop last row — it is the *current forming* 5m candle
            if len(df) > 1:
                df = df.iloc[:-1].copy()
...
def get_full_signal(symbol):
    try:
        ...
        # === LAST ROW: use the *latest completed* 5m candle (PRICE MATCH FIX - EXACT MATCH TO INDICATOR) ===
        # We use iloc[-1] (the most recent *completed* bar after yf download).
        # This ensures the alert uses:
        #   - entry_time + price = close of the EXACT candle that triggered the BUY/SELL on your TradingView Pine v6.3 chart
        #
        # This matches the visual trigger candle you see on chart (BUY REVERSAL3 / ORB break etc).
        # (Removed old iloc[-2] "delay" logic to achieve 100% price/time match with indicator.)
        if len(df) < 2:
            return None, "not enough 5m data for completed candle", None

        last = df.iloc[-1]

        # yfinance .NS 5m: 'timestamp' = START time of the completed candle (Asia/Kolkata)
        # The candle closes 5 min later. We use this bar's close for entry/alert.
        ...
        print(f"      [ALERT BAR] start_ist={bar_start.strftime('%H:%M')} | close_ist={bar_close.strftime('%H:%M')} | close_price={last['close']:.2f}")
