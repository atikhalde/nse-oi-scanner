...
def fetch_5m_data(symbol: str):
    try:
        ...
        if len(df) >= 25:
            ...
            # yfinance .NS → Asia/Kolkata, timestamp = START of 5m candle
            if df['timestamp'].dt.tz is None:
                df['timestamp'] = df['timestamp'].dt.tz_localize('Asia/Kolkata')
            else:
                df['timestamp'] = df['timestamp'].dt.tz_convert('Asia/Kolkata')

            # CRITICAL: Drop the last row — it is the *current forming* candle
            if len(df) > 1:
                df = df.iloc[:-1].copy()
...
def get_full_signal(symbol):
    try:
        ...
        # yfinance .NS: timestamp = START of the completed 5m candle (Asia/Kolkata)
        ...
        print(f"      [OI+F&O BAR] start_ist={bar_start.strftime('%H:%M')} | close_ist={bar_close.strftime('%H:%M')} | close_price={last['close']:.2f}")
...
if __name__ == "__main__":
    ...
    if run_once:
        print("🔹 RUN_ONCE mode (GitHub Actions one-shot) - OI + F&O + Indicator")
        now = datetime.now(IST)
        current = now.strftime("%H:%M")
        ...
        test_msg = (
            "🧪 PAPER TRADER (OI+F&O) TEST (RUN_ONCE)\n\n"
