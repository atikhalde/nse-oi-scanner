...
check("A: NO fixed-target legs (exits v3 pure runner)", not any(l[0].startswith(("T1","T2")) for l in tr["legs"]) and all(l[1] > 0 for l in tr["legs"]))
check("A: full qty in one exit (no partials)", sum(l[1] for l in tr["legs"]) == tr["qty"])
...
# ---------- G. prevbar SL mode (blocked weak-EX family GHOST shadow rule)
dayG = mk_bars([
    (100.0, 100.5, 99.5, 100.2),
    (100.2, 100.8, 99.9, 100.5),   # prev bar: low 99.9
    (100.5, 101.2, 100.1, 101.0),  # entry bar, close 101
    (101.0, 101.5, 100.8, 101.2),
    (101.2, 101.4, 99.0, 99.4),    # breaches the prev-bar-low stop
    (99.4, 99.6, 99.0, 99.2),
])
trG = trader.evaluate("TESTG", "BUY", dayG["t"].iloc[2], 101.0, "TEST", dayG, warmup=warm(120, 98, 0.02), sl_mode="prevbar")
check("G: BUY prevbar SL = prev candle low -0.02%", abs(trG["sl"] - 99.9 * (1 - 0.0002)) < 0.01)
check("G: sl_mode recorded in trade dict", trG["sl_mode"] == "prevbar")
check("G: prevbar stop exits the breach bar", trG["closed"] and trG["pnl"] < 0)
trG0 = trader.evaluate("TESTG", "BUY", dayG["t"].iloc[2], 101.0, "TEST", dayG, warmup=warm(120, 98, 0.02))
check("G: default mode stays structure (live trades unchanged)", trG0["sl_mode"] == "structure")

# SELL mirror: SL = prev candle high +0.02%
dayH = mk_bars([
    (100.0, 100.6, 99.8, 100.1),
    (100.1, 100.9, 99.9, 100.3),   # prev bar: high 100.9
    (100.3, 100.7, 99.2, 99.5),    # entry bar, close 99.5
    (99.5, 100.0, 99.0, 99.3),
    (99.3, 101.2, 99.1, 101.0),    # rallies through the stop
    (101.0, 101.2, 100.4, 100.8),
])
trH = trader.evaluate("TESTH", "SELL", dayH["t"].iloc[2], 99.5, "TEST", dayH, warmup=warm(120, 102, -0.02), sl_mode="prevbar")
check("G: SELL prevbar SL = prev candle high +0.02%", abs(trH["sl"] - 100.9 * (1 + 0.0002)) < 0.01)
check("G: SELL prevbar stop exits the breach bar", trH["closed"] and trH["pnl"] < 0)

# degenerate case: prev candle sits entirely beyond entry on the risk side -> entry-bar fallback
dayGD = mk_bars([
    (103.0, 103.5, 102.8, 103.2),
    (103.2, 103.6, 102.5, 103.0),  # prev bar low 102.5 = ABOVE the coming entry close
    (101.0, 101.3, 100.2, 101.0),  # gap-down entry bar, close 101
    (101.0, 101.6, 100.8, 101.4),
    (101.4, 101.5, 100.6, 100.9),
])
trGD = trader.evaluate("TESTGD", "BUY", dayGD["t"].iloc[2], 101.0, "TEST", dayGD, warmup=warm(120, 104, -0.01), sl_mode="prevbar")
check("G: degenerate prev bar -> entry-bar low fallback", abs(trGD["sl"] - 100.2 * (1 - 0.0002)) < 0.01 and "entry-bar" in trGD["sl_anchor"])
