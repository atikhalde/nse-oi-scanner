...
Targets (partial booking): 50% @ 1:2 · 30% @ 1:3 · final 20% trailed.
...
TP1_R, TP1_FRAC = 2.0, 0.50
TP2_R, TP2_FRAC = 3.0, 0.30
TRAIL_ARM_R = 1.0
NEAR_ATR = 0.30
...
def evaluate(sym, side, etime, entry, signal, bars, warmup=None, today_date=None):
    ...
    for lbl, q, px, t in legs:
        if lbl.startswith("T1"):
            parts.append(f"50% TP@1:2 {t}")
        elif lbl.startswith("T2"):
            parts.append(f"30% TP@1:3 {t}")
...
def fmt_alert(tr, key):
    ...
    if key == "ENTRY":
        ...
        return (f"🚨 ENTRY · {base}\n"
                ...
                f"T1 50% @ ₹{tr['tp1']} (1:2) · T2 30% @ ₹{tr['tp2']} (1:3) · rest 20% trails after +1R")
    if key == "TP1":
        return f"💰 T1 BOOKED · {base}\n50% @ ₹{tr['tp1']} (+2R) · 50% left, trail arms at +1R"
    if key == "TP2":
        return f"💰💰 T2 BOOKED · {base}\n30% @ ₹{tr['tp2']} (+3R) · last 20% now on {tr.get('trail_style', 'trail')}"
    if key == "TRAIL_ON":
