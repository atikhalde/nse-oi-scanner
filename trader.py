...
Targets: NONE — 24-Jul rule. Structure SL ∓0.02%; after +1R the full position rides
# the structure-swing trail to 15:20 (pure runner; 2-day evidence: +24% net vs partials).
...
CAPITAL = 50000          # notional per trade = Rs10,000 MIS margin x5 (user spec)
MARGIN_RS = 10000          # real cash consumed per trade (paper label only)
BUFFER = 0.0002
SQOFF = "15:20"
RISK_CAP = 900          # planned max loss (Rs1,000 cap - Rs100 gap buffer) — user spec
TP1_R, TP1_FRAC = 99.0, 0.00     # NO fixed targets (24-Jul rule): winners ride
TP2_R, TP2_FRAC = 99.0, 0.00     # the structure trail to 15:20 — no partial booking
TRAIL_ARM_R = 1.0
NEAR_ATR = 0.30
...
def evaluate(sym, side, etime, entry, signal, bars, warmup=None, today_date=None, sl_mode="structure"):
    ...
    sl_mode: "structure" (default, setup-aware) or "prevbar" — released-family rule
    (24-Jul user spec): SL = PREVIOUS candle's low (BUY) / high (SELL) ∓0.02%;
    falls back to the entry candle's low/high when the previous bar is degenerate."""
    ...
    if sl_mode == "prevbar":
        i0 = max(0, ei - 1)
        plo = float(bars["low"].iloc[i0]); phi = float(bars["high"].iloc[i0])
        psl = plo * (1 - BUFFER) if side == "BUY" else phi * (1 + BUFFER)
        anchor = "prev-bar low/high ∓0.02%"
        if (side == "BUY" and psl >= entry) or (side == "SELL" and psl <= entry):
            psl = float(bars["low"].iloc[ei]) * (1 - BUFFER) if side == "BUY" \
                else float(bars["high"].iloc[ei]) * (1 + BUFFER)
            anchor = "entry-bar low/high ∓0.02% (prev-bar degenerate)"
        sl = psl
    ...
    for j in range(ei + 1, len(bars)):
        ...
        if (s == 1 and l <= stop) or (s == -1 and h >= stop):
            ...
            break
        # 2) partial books
        if q1 > 0 and not got1 and ((s == 1 and h >= tp1) or (s == -1 and l <= tp1)):
            got1 = True
    ...
    return {
        "symbol": sym, "side": side, "time": etime, "signal": signal, "setup": cls, "sl_mode": sl_mode,
...
def fmt_alert(tr, key):
    ...
    if key == "ENTRY":
        ...
        return (f"🚨 ENTRY · {base}\n"
                ...
                f"NO fixed targets — 100% rides structure trail after +1R to 15:20 · notional ₹{tr['capital']:,.0f} (MIS 5× · ₹10,000 margin)")
    if key == "TP1":
    ...
    if key == "TRAIL_ON":
        return f"🧲 RUNNER ON · {base}\n+1R printed — 100% now trails {tr.get('trail_style', 'structure')} to 15:20 (no targets)"
    if key == "EXIT_SL":
